...
<h3>LAPORAN KEUANGAN SMP SEPULUH NOPEMBER SIDOARJO</h3>
...
<table  border="1" cellspacing="0" width="600">
    ...
    <tr><td>KELAS</td><td>: <?php echo strtoupper(getField('akademik_prodi', 'nama_prodi', 'prodi_id', $prodi_id))?></td></tr>
</table>
<BR>
<table border="1" cellspacing="0" bordercolor="#7E7E7E">
    ...
    $no=1;
    $tunggakan_total=0;
    $mahasiswa=$this->db->query("select nim,nama from student_mahasiswa where prodi_id='$prodi_id' and angkatan_id='$t->angkatan_id'")->result();
...
<Br>
<?php
$jml_semester=$this->db->query("SELECT max(semester_aktif) as max_semester FROM `student_mahasiswa` WHERE prodi_id='$prodi_id' and angkatan_id='$t->angkatan_id'")->row_array();
...
<table border="1" cellspacing="0" bordercolor="#7E7E7E">
    ...
    $no=1;
    $tunggakan_total=0;
    $mahasiswa=$this->db->query("select nim,nama from student_mahasiswa where prodi_id='$prodi_id' and angkatan_id='$tahun_akademik'")->result();
    foreach ($mahasiswa as $m)
    {
        echo "<tr><td>$no</td>
            <td>".  strtoupper($m->nim)."</td>
...
<Br>
<?php
$jml_semester=$this->db->query("SELECT max(semester_aktif) as max_semester FROM `student_mahasiswa` WHERE prodi_id='$prodi_id' and angkatan_id='$tahun_akademik'")->row_array();
$jml_semester=$jml_semester['max_semester'];
?>
<table border="1" cellspacing="0" bordercolor="#7E7E7E">
    <tr><th>No</th><th>NIM</th><th>NAMA</th>
