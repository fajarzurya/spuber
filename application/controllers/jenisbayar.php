...
class jenisbayar extends CI_Controller{
	...
	foreach ($record as $r) 
            {
                ...
                $edit           =array('data' => anchor($this->uri->segment(1).'/edit/'.$r->$pk,'<i class="fa fa-search"></i>'), 'width' => 10,'title'=>'Edit Data');
                $this->table->add_row(
                        $no,
	...
	$tmpl = array ( 'table_open'  => '<table border="0" cellpadding="0" cellspacing="0" class="table table-responsive table-hover">' );
